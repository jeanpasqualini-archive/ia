...
**The ground is raised, not shaded.** `Map\Relief` keeps the elevation field `TerrainMapProvider` cuts the terrain out of and used to discard, so terrain and relief agree by construction — no lake sits on a hilltop, because being low is what made it a lake. It never enters `World`: a snapshot costs eight kilobytes and forty thousand heights would undo that in one line. The map view shades by the slope, the isometric view lifts the tile and drops its silhouette straight down as the side of a block. The lift is deliberately small — ten pixels across the whole range, on a diamond eight pixels tall — because an earlier attempt raised a wood ten *rows* above the meadow and the picture shredded: one read a torn map rather than a hill.

**Water is never raised and never shaded.** The field carries on below the water line, so lifting a lake by it draws the bottom of the lake as though that were its surface, and a bay comes out with a hillside in it.
...
**The isometric view never samples, and `z` there means a bigger tile.** Its coverage is a diamond of some seventy cells a side, so sampled at 1:4 it asks the world for nearly three hundred rows where there are a hundred and sixty: everything past the edge is skipped and the picture closes into a wedge with sky around it — measured, 41% of the screen at 1:4 and 92% at 1:8. Sampling would say nothing there in any case, a tree standing for eight tiles being no kind of answer. So closer means the buffer is composed on a quarter of the area instead of a half and blown up by four: the same art, 64 pixels a tile instead of 32, and **cheaper rather than dearer** — 5.1 ms against 19.6, there being a quarter as many tiles to paint.

Zooming therefore means something different in each view, which is why `GameRenderInterface::zoom()` exists rather than a flag the loop reads: the loop deciding would mean the loop knowing which view is on. Entering the isometric view gives up the map's sampled zoom and remembers it — captured on the way *out* of the map and nowhere else, since recomputing it on every swap resets it to zero the second time round and the map's zoom is quietly lost.

**A first person view was built and removed.** A ray caster — one ray per screen column walked across the tile grid, Wolfenstein's algorithm, and cheap precisely because the world is already a grid of tiles with a cost per tile. It worked, at 14.5 ms for 256x160, and it was thrown away after being looked at: seen from inside, this world has nothing to watch. What makes a cat worth following is the shape of what it does over a hundred tiles, and standing in its shoes shows a wall, a lake and the next few metres. Observability here is a wide view, not an immersive one — which is not something that could have been settled by argument, only by opening the window.

Its one useful residue is a measurement: the rays cost nothing and the *floor* was the whole of the 14.5 ms, forty thousand pixels each asking the palette what lies there. The visible half of a problem was not the expensive half.

Two more the relief brought, both of which tore the landscape open. The side of a block was tapered by a pixel a row, which finishes in a point and leaves two black wedges at every raised tile's corners; it has to be the silhouette dropped *straight* down, which lands back on the plane an unraised tile would sit on and is what closes the gaps. And `$screenY` is computed once per depth, so subtracting the lift from it inside the column loop raised each tile by the sum of the heights of every tile drawn before it on the same row — the landscape drifted upward across each row. A smooth ramp hides both, because neighbours are lifted alike: `testNoHoleOpensBetweenTwoTilesOfRaisedGround` uses a real seeded terrain, and passed on a ramp while a seventh of the picture was sky.
